...
class ConstantInput(nn.Module):
    def __init__(self, channel, size=4):
        ...
        self.input = nn.Parameter(torch.randn(1, channel, 5, 40))
...
class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        ...
        self.final_linear = nn.Sequential(
            EqualLinear(channels[4] * 5 * 40 , channels[4], activation="fused_lrelu"),
...
if __name__ == "__main__":
    ...
    fake_img2 = torch.randn(2, 1,80, 640).cuda()

    dis = Discriminator(size=64).cuda()
    fake_output = dis(fake_img)
    fake_output2 = dis(fake_img2)
